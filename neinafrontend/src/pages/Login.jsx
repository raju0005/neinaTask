...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://neina-task-server.vercel.app/api/auth/v1/login', credentials);
  ...
  useEffect(()=>{
    const fetch = async () => {
    const res = await axios.get('https://neina-task-server.vercel.app/api/auth')
    console.log(res)}
    fetch()
  })
